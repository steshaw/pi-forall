...
tcTerm (Pi bnd) Nothing = do
  ...
  pure (Pi (bind (x, embed atyA) atyB), Type)
...
tcTerm (Lam bnd) (Just (Pi bnd2)) = do
  ...
  maybe (pure ()) (equate tyA) ma
  -- check the type of the body of the lambda expression
  (ebody, etyB) <- extendCtx (Sig x tyA) (checkType body tyB)
  pure (Lam (bind (x, embed (Annot (Just tyA))) ebody),
          Pi bnd2)
...
tcTerm (Lam bnd) Nothing = do
  ...
  tyA  <- maybe (err [DS "Must annotate lambda"]) (pure) annot
  ...
  pure (Lam (bind (x, embed (Annot (Just atyA))) ebody),
...
tcTerm t@(TrustMe ann1) ann2 = do
  ...
  pure (TrustMe (Annot (Just expectedTy)), expectedTy)

tcTerm (TyUnit) Nothing = pure (TyUnit, Type)

tcTerm (LitUnit) Nothing = pure (LitUnit, TyUnit)

tcTerm (TyBool) Nothing = pure (TyBool,Type)

tcTerm (LitBool b) Nothing = pure (LitBool b, TyBool)
...
tcTerm t@(If t1 t2 t3 ann1) ann2 = do
  ...
  pure (If at1 at2 at3 (Annot (Just ty)), ty)
...
tcTerm (Let bnd) ann = do
  ...
  pure (Let (bind (x,embed arhs) abody), ty)

tcTerm (TyEq a b) Nothing =  do
  (aa,aTy) <- inferType a
  (ab,bTy) <- checkType b aTy
  pure (TyEq aa ab, Type)
...
tcTerm t@(Subst tm p ann1) ann2 =  do
  ...
  edecl <- do
    ...
    n'        <- whnf n
    case (m',n') of
        (Var x, _) -> pure [Def x n']
        (_, Var y) -> pure [Def y m']
        (_,_) -> pure []

  pdecl <- do
    p'        <- whnf apf
    case p' of
      (Var x) -> pure [Def x (Refl (Annot (Just tp)))]
      _       -> pure []
  let refined = extendCtxs (edecl ++ pdecl)
  (atm, _) <- refined $ checkType tm ty
  pure (Subst atm apf (Annot (Just ty)), ty)
...
tcTerm t@(Contra p ann1) ann2 = do
  ...
  case (a',b') of
    ...
    (LitBool b1, LitBool b2) | b1 /= b2 ->
      pure (Contra apf (Annot (Just ty)), ty)
...
tcTerm t@(Sigma bnd) Nothing = do
  ...
  pure (Sigma (bind (x,embed aa) ba), Type)
...
tcTerm t@(Prod a b ann1) ann2 = do
  ...
  case ty of
     (Sigma bnd) -> do
      ...
      pure (Prod aa ba (Annot (Just ty)), ty)
  ...
  case pty' of
    Sigma bnd' -> do
      ...
      (abody, bTy) <- extendCtxs ([Sig x' tyA, Sig y' tyB'] ++ ctx) $
        checkType body ty
      pure (Pcase apr (bind (x',y') abody) (Annot (Just ty)), bTy)
...
matchAnnots e (Annot Nothing) (Just t)    = pure t
...
tcModule defs m' = do checkedEntries <- extendCtxMods importedModules $
                                          foldr tcE (pure [])
                                                  (moduleEntries m')
                      pure $ m' { moduleEntries = checkedEntries }
  where d `tcE` m = do
    ...
    tc = do
      ...
      case lkup of
        Nothing -> do (aterm, ty) <- inferType term
                      pure $ AddCtx [Sig n ty, Def n aterm]
        Just ty ->
          ...
          in do
            ...
            if (n `elem` toListOf fv eterm) then
                 pure $ AddCtx [Sig n ety, RecDef n eterm]
              else
                 pure $ AddCtx [Sig n ety, Def n eterm]
    die term' =
